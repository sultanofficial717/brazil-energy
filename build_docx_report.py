...
def create_report():
    ...
    # Page setup - Standard Letter, 0.85 inch margins for clean academic proportion
    ...
    for section in sections:
        ...
        hrun.font.color.rgb = RGBColor(113, 128, 150) # Slate
    ...
    def set_run_font(run, name='Calibri', size_pt=10.0, bold=False, italic=False, color=COLOR_BODY):
        run.font.name = name
    ...
    def add_h1(text, space_before=10, space_after=3):
        p = doc.add_paragraph()
        p.paragraph_format.space_before = Pt(space_before)
        p.paragraph_format.space_after = Pt(space_after)
        p.paragraph_format.keep_with_next = True
        run = p.add_run(text)
        set_run_font(run, name='Calibri', size_pt=13.5, bold=True, color=COLOR_PRIMARY)
        return p

    def add_h2(text, space_before=8, space_after=2):
        p = doc.add_paragraph()
        p.paragraph_format.space_before = Pt(space_before)
        p.paragraph_format.space_after = Pt(space_after)
        p.paragraph_format.keep_with_next = True
        run = p.add_run(text)
        set_run_font(run, name='Calibri', size_pt=11.0, bold=True, color=COLOR_SECONDARY)
        return p

    def add_body(text, bold_prefix=None, italic_prefix=None, space_after=3.0):
        p = doc.add_paragraph()
    ...
    def add_bullet(text, bold_prefix=None, space_after=1.5):
    ...
    def add_callout(text, bold_title="Key Takeaway: "):
        ...
        set_run_font(r_t, size_pt=9.0, bold=True, color=COLOR_PRIMARY)
        r = p.add_run(text)
        set_run_font(r, size_pt=9.0, italic=False, color=COLOR_BODY)
        return p
        ...
        for row_idx, row in enumerate(table.rows):
            ...
            for col_idx, cell in enumerate(row.cells):
                ...
                for p in cell.paragraphs:
                    ...
                    for run in p.runs:
                        if row_idx == 0:
                            set_run_font(run, size_pt=8.5, bold=True, color=RGBColor(255, 255, 255))
                        else:
                            set_run_font(run, size_pt=8.5, color=COLOR_BODY)

    def add_image_with_caption(img_path, caption_num, caption_text, width_in=5.0):
        ...
        set_run_font(r_num, size_pt=8.0, bold=True, color=COLOR_PRIMARY)
        
        r_txt = p_cap.add_run(caption_text)
        set_run_font(r_txt, size_pt=8.0, italic=True, color=COLOR_MUTED)
        return p_cap
    ...
    meta_p.paragraph_format.line_spacing = 1.30
    ...
    # =========================================================================
    add_h1("3. Introduction & Research Problem")
    add_body(
        "In the Brazilian wholesale power market, electricity dispatch is managed centrally by the National System Operator "
        "(ONS) using mathematical optimization algorithms (primarily NEWAVE and DECOMP) that compute shadow prices based on hydro "
    ...
    # =========================================================================
    # 4. DATA & TARGET VARIABLES
    # =========================================================================
    add_h1("4. Data & Target Variables")
    add_body(
    ...
    set_run_font(r, size_pt=9.0, bold=True, color=COLOR_PRIMARY)
    ...
    format_table(t1, col_widths=[1.6, 5.2], alignments=[WD_ALIGN_PARAGRAPH.LEFT, WD_ALIGN_PARAGRAPH.LEFT])
    ...
    # =========================================================================
    add_h1("6. Official Benchmark Results")
    add_body(
    ...
    # =========================================================================
    # 7. VISUAL RESULTS & EMPIRICAL EVIDENCE
    # =========================================================================
    add_h1("7. Visual Results & Empirical Evidence")
    add_body(
        "This section presents the empirical visual proofs generated by the project's analytical and plotting routines. "
        "Figures 1–4 detail the exploratory structure of the Brazilian power grid and news narratives, while Figures 5–8 document "
        "out-of-sample forecasting performance and embedding behavior."
    )
    
    add_h2("A. Exploratory Market Dynamics & News Narratives")
    
    # Figure 1
    add_image_with_caption(
        "figures/fig1_timeseries_gap_dynamics.png",
        caption_num=1,
        caption_text="Brazilian Power Market Dynamics & Target Gap (2018–2026). Top panel displays CMO and PLD series highlighting "
                     "the 2020–2021 water crisis window and regulatory price limits. Bottom panel depicts Target Gap basis dislocations.",
        width_in=4.8
    )
    add_body(
        "Figure 1 illustrates the long-term price trajectory across the 2018–2026 scope. The 2020–2021 COVID-19 and extreme hydrological drought "
        "pushed both PLD and CMO against regulatory ceilings (~584–700 R$/MWh). The bottom panel reveals severe basis dislocations, "
        "where the CMO−PLD gap spiked beyond 600 R$/MWh, confirming the non-linear volatility of the underlying target."
    )
    ...
    add_image_with_caption(
        ...
        caption_num=2,
        caption_text="Target Distribution Comparison Across Regimes. (A) Target spread in the full dataset (σ = 221.0) versus "
                     "crisis-filtered data (σ = 79.2). (B) Boxplot distributions across four distinct regulatory temporal sets.",
        width_in=4.8
    )
    add_body(
        "Figure 2 contrasts the dispersion of the target basis gap across market regimes. Extreme drought years (Set 2) exhibit a massive standard "
        "deviation (σ = 221.0), whereas post-crisis and modern hourly regimes (Sets 3 and 4) exhibit tighter dispersion (σ = 79.2), proving the need "
        "for stationary delta modeling to handle structural regime shifts."
    )
    ...
    # Figure 3
    add_image_with_caption(
        "figures/fig3_regional_market_breakdown.png",
        caption_num=3,
        caption_text="Regional Submarket Breakdown. (A) Regional mean PLD and CMO across the four SIN regions. "
                     "(B) Target basis gap volatility (standard deviation) highlighting inter-regional basis risk.",
        width_in=4.8
    )
    add_body(
        "Figure 3 establishes regional price heterogeneity. While Southeast and Central-West submarkets exhibit high target volatility, "
        "the North and Northeast regions experience structural price decoupling during wet season export congestion, validating the incorporation "
        "of regional load spreads in the feature space."
    )

    # Figure 4
    add_image_with_caption(
        "figures/fig4_news_topic_sentiment_temporal.png",
        caption_num=4,
        caption_text="Energy News Narrative Trajectories & Sentiment (2018–2026). Top panel shows monthly mentions of Drought, Hydro Levels, "
                     "Flood, and Curtailment. Bottom panel tracks average news sentiment score.",
        width_in=4.8
    )
    add_body(
        "Figure 4 tracks domain news activity across the eight-year research timeline. Mentions of drought and hydro reservoir stress surged "
        "sharply during the 2020–2021 crisis, accompanied by a precipitous decline in average sentiment. Renewable curtailment mentions expand "
        "rapidly in 2024–2026, marking a modern transition in market risk narratives."
    )

    doc.add_page_break()

    add_h2("B. Out-of-Sample Forecasting Performance & Embeddings")
    ...
    add_body(
        "Figure 5 displays the four official benchmark subplots. The Full Pipeline (dark blue bar) consistently surpasses both Market Only "
    ...
    add_body(
        "Figure 6 overlays model predictions directly against actual price levels across the out-of-time evaluation timeline. The Full Pipeline "
    ...
    add_body(
        "Figure 7 evaluates 12-hour sub-daily price momentum tracking. Panel 2 demonstrates that the Full Pipeline accurately captures "
    ...
    add_body(
        "Figure 8 details the empirical mechanics of the embedding representations. Supervised PLS Component 1 demonstrates a strong linear "
    ...
    # =========================================================================
    add_h1("8. Statistical Validation")
    add_body(
    ...
    p_t3.paragraph_format.space_before = Pt(3)
    ...
    # =========================================================================
    add_h1("9. Conclusion & Limitations")
    add_body(
    ...
    # 10. REPRODUCIBILITY
    # =========================================================================
    add_h1("10. Reproducibility")
    add_body(
        "The project repository is structured for end-to-end reproducibility across four primary directories:"
    )
    add_bullet("Contains all 12 publication-quality PNG benchmark visualizations and diagnostic plots.", bold_prefix="figures/: ")
    ...
    add_bullet("Regenerates the official benchmark comparison (Figure 5) and chronological timeline overlay (Figure 6).", bold_prefix="scripts/generate_perfectly_matched_visuals.py: ")
    add_bullet("Generates 12-hour price delta momentum plots (Figure 7) and the interactive HTML visualization dashboard.", bold_prefix="scripts/generate_12h_visualizations.py: ")
    ...
    add_bullet("Compiles Figures 1–4 and generates the comprehensive visual data analysis master report.", bold_prefix="scripts/generate_complete_data_analysis_pdf.py: ")
    ...
    p_cite.paragraph_format.space_after = Pt(4)
    p_cite.paragraph_format.left_indent = Inches(0.15)
    p_cite.paragraph_format.right_indent = Inches(0.15)
    ...
    set_run_font(r_cite, name='Courier New', size_pt=8.0, color=COLOR_BODY)
    ...
    print(f"Successfully generated research report: {output_filename}")
