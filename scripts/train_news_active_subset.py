...
print("="*80)
print("BENCHMARK ON NEWS-ACTIVE SUBSET (ONLY DAYS WITH NEWS TOPICS & ARTICLES > 0)")
print("="*80)
...
# 2. FILTER OUT 2021-2022 CRISIS & KEEP ONLY NEWS-ACTIVE DAYS
df_filt = df[~df['Date_dt'].dt.year.isin([2021, 2022])].copy()
df_news = df_filt[df_filt['Articles'] > 0].copy().reset_index(drop=True)

print(f"   - Total rows in 2018-2020 & 2023-2026: {len(df_filt)}")
print(f"   - Filtered News-Active rows (Articles >= 1): {len(df_news)} (100% have valid news text & embeddings)")
print(f"   - Mean Articles/Day: {df_news['Articles'].mean():.2f} (Max: {df_news['Articles'].max()})")

# 3. FEATURE ENGINEERING
print("\n2. Engineering Market Spreads, Volatilities, and Dynamics on News Subset...")
...
print(f"   - Test Target Variance: {y_test_level.var():.2f}")
...
print("\n4. Training & Evaluating Models Across All 4 Configurations on News Subset...")

def evaluate_model(name, train_x, test_x, model_type='ensemble'):
    if model_type == 'rf':
        m = RandomForestRegressor(n_estimators=300, max_depth=20, min_samples_leaf=2, max_features=0.75, random_state=42, n_jobs=-1)
        m.fit(train_x, y_train_delta)
        pred_d = m.predict(test_x)
    elif model_type == 'hgb':
        m = HistGradientBoostingRegressor(max_iter=300, max_depth=8, learning_rate=0.03, l2_regularization=1.0, random_state=42)
        m.fit(train_x, y_train_delta)
        pred_d = m.predict(test_x)
        ...
        m = m_rf  # for feature importances
...
print("   - Training Config 1: Market Only (Without any News or Embeddings)...")
res_mkt = evaluate_model("1. Market Only (No News/Embeddings)", X_train_mkt, X_test_mkt, 'ensemble')

print("   - Training Config 2: Market + News Topic Counts & Sentiment...")
res_topics = evaluate_model("2. Market + News Topic & Sentiment", X_train_topics, X_test_topics, 'ensemble')

print("   - Training Config 3: Full Pipeline (Market + Topics + BGE News Embeddings)...")
res_full = evaluate_model("3. Full Pipeline (+ BGE News Embeddings)", X_train_full, X_test_full, 'ensemble')

print("   - Training Config 4: News Alone (Topics + Embeddings Only)...")
res_news_only = evaluate_model("4. News Alone (Topics + Embeddings)", X_train_news_only, X_test_news_only, 'ensemble')
...
all_results = {
    ...
    "Timeline": "2018-2020 & 2023-2026 (Excluding 2021-2022 Crisis)",
    "Total_Rows": len(df_news),
    "Train_Rows": len(y_train_delta),
    "Test_Rows": len(y_test_delta),
    "Comparison": [
        {k: v for k, v in res_mkt.items() if k != "Model"},
        {k: v for k, v in res_topics.items() if k != "Model"},
        {k: v for k, v in res_full.items() if k != "Model"},
        {k: v for k, v in res_news_only.items() if k != "Model"}
    ],
    "Top_20_Feature_Importances": imp_series.head(20).to_dict()
}

with open(get_result_path("news_active_subset_benchmark_results.json"), "w") as f:
    json.dump(all_results, f, indent=4)

# 9. PRINT COMPARISON TABLE
print("\n" + "="*95)
print("FINAL BENCHMARK COMPARISON ON NEWS-ACTIVE SUBSET (ARTICLES >= 1)")
print("="*95)
...
print("\nTop 15 Feature Importances in Full Pipeline (News-Active Subset):")
for k, v in imp_series.head(15).items():
    print(f"   - {k:<34}: {v:.6f} ({v*100:.2f}%)")

print("\nBenchmark completed successfully! Saved to news_active_subset_benchmark_results.json")
