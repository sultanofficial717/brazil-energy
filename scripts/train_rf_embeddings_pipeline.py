...
print("\nParsing BGE news text embeddings (1024 dimensions)...")
...
print(f"Total dataset: {len(df)} rows | Train: {len(y_train)} rows | Test: {len(y_test)} rows")

# Apply PCA on BGE embeddings to capture dense latent semantic factors
...
train_pca_bge = pd.DataFrame(pca_bge.transform(bge_df.iloc[:split_idx]), columns=pca_bge_cols, index=df.index[:split_idx])
test_pca_bge = pd.DataFrame(pca_bge.transform(bge_df.iloc[split_idx:]), columns=pca_bge_cols, index=df.index[split_idx:])
...
# ==========================================
X_train_mkt = df.iloc[:split_idx][hist_market_cols]
X_test_mkt = df.iloc[split_idx:][hist_market_cols]
...
# ==========================================
X_train_topics = df.iloc[:split_idx][hist_market_cols + news_topic_cols]
X_test_topics = df.iloc[split_idx:][hist_market_cols + news_topic_cols]
...
# ==========================================
X_train_full = pd.concat([df.iloc[:split_idx][hist_market_cols + news_topic_cols], train_pca_bge], axis=1)
X_test_full = pd.concat([df.iloc[split_idx:][hist_market_cols + news_topic_cols], test_pca_bge], axis=1)
...
with open(get_result_path("rf_news_embeddings_benchmark_results.json"), "w") as f:
    json.dump(results, f, indent=4)

print("\n" + "="*70)
print("RANDOM FOREST PIPELINE BENCHMARK RESULTS")
print("="*70)
...
print("\nBenchmark completed and saved to rf_news_embeddings_benchmark_results.json")
